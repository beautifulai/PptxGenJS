...
import * as paragraph from './generators/paragraph';
import textRun from './generators/textRun';
...
export function genXmlTextRun (textObj: TextProps): string {
	...
	const textRuns = [];
	let xmlTextRun = '';
	const xmlProperties = textRun.properties.generateXml(textObj.options, false);
...
export function genXmlTextBody (slideObj: ISlideObject | TableCell, slide: PresSlide | SlideLayout): string {
	...
	// STEP 2: Add bodyProperties
	{
		...
		else if (slideObj._type === 'placeholder') strSlideXml += `<a:lstStyle>${paragraph.generateXml(slideObj, true, slide)}</a:lstStyle>`;
	...
	*/
	...
	arrLines.forEach(line => {
		...
		line.forEach((textObj, idx) => {
			...
			paragraphPropXml = paragraph.generateXml(textObj, false, slide);
